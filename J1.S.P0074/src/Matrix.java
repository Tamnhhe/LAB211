...
class Matrix {
    ...
    public int[][] getFirstMatrix() {
        ...
        int row = Validation.getInt(Message.format(Message.ENTER_ROW_MATRIX, 1), Message.ERROR_ROW_MATRIX, Message.ERROR_INVALID_NUMBER, 1, Integer.MAX_VALUE);
        int column = Validation.getInt(Message.format(Message.ENTER_COLUMN_MATRIX, 1), Message.ERROR_COLUMN_MATRIX, Message.ERROR_INVALID_NUMBER, 1, Integer.MAX_VALUE);
    ...
    public int[][] getSecondMatrix(int[][] firstMatrix, char operator) {
        ...
        if (operator == Constant.ADDITION || operator == Constant.SUBTRACTION) {
            ...
            columnSecondMatrix = Validation.getInt(Message.format(Message.ENTER_COLUMN_MATRIX, 2), Message.COLUMN_NOT_MATCH, Message.ERROR_INVALID_NUMBER, columnFirstMatrix, columnFirstMatrix);
        }
        ...
        if (operator == Constant.MULTIPLICATION) {
            ...
            columnSecondMatrix = Validation.getInt(Message.format(Message.ENTER_COLUMN_MATRIX, 2), Message.COLUMN_SHOULD_BE_POSITIVE, Message.ERROR_INVALID_NUMBER, 1, Integer.MAX_VALUE);
        }
